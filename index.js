...
// خدعة المتصفح الشاملة لتخطي حماية المواقع
const headers = {
...
app.get('/', (req, res) => {
    res.status(200).send('السيرفر الشامل يعمل بنجاح ويدعم مواقع متعددة! 🚀');
});

// 1. جلب قائمة المانجا (يدعم Olympus و Lek Manga وغيرها)
app.get('/api/scrape', async (req, res) => {
    ...
    try {
        ...
        // 💡 ضفنا كلاسات جديدة خاصة بـ Olympus Staff وباقي المواقع المشهورة
        const templateSelectors = [
            '.bsx', '.page-item-detail', '.manga-card', '.item', 
            '.post-item', '.mangacard', '.page-listing-item', 
            '.c-tabs-item__content', '.series-box', '.manga-item'
        ];
        ...
        for (let selector of templateSelectors) {
            if ($(selector).length > 0) {
                ...
                break; // إذا لقينا التصميم الصح، نوقف البحث
            }
        }
...
// 2. جلب تفاصيل المانجا (القصة والفصول)
app.get('/api/details', async (req, res) => {
    ...
    try {
        ...
        let chapters = [];
        // 💡 كلاسات الفصول لموقع Olympus وغيره
